...
abstract class TweetSet {
  ...
  def mostRetweeted: Tweet =
    rList.mostRetweeted(getTweet)
  
  def mostRetweeted(tw: Tweet): Tweet = 
    if(this.isEmpty) tw
    else if (getTweet.retweets>tw.retweets) rList.mostRetweeted(getTweet)
    else rList.mostRetweeted(tw)
...
class Empty extends TweetSet {
  ...
  override def rList: TweetSet = this
  override def getTweet: Tweet = new Tweet("","",0)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def getTweet: Tweet = if (left.isEmpty) elem else left.getTweet
  override def rList: TweetSet = if (left.isEmpty) right else new NonEmpty(elem,left.rList,right)
  /**
   * The following methods are already implemented
   */
